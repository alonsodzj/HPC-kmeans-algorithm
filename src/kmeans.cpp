...
void kmeans_mpi(const std::vector<float>& datos_local, size_t n_filas_local, size_t n_columnas, std::vector<float>& centroides, std::vector<int>& asignaciones_local)
{
    ...
    while (iteraciones < 2000 && !calidad)
    {
        ...
        {
            //sección paralela
            std::vector<float> suma_thread(centroides.size(), 0.0f);
            //creo un hilo que acumula sumas
            std::vector<int> conteo_thread(num_centroides, 0);
            //creo un hilo que lleva la guenta 

            int desplazados_thread = 0; //cada hilo tiene una variable de estas

            #pragma omp for nowait  //evita la barrera implícita al final del bucle por lo que si acabo mis iteraciones sigo
            ...
            //por cada punto
            {
                ...
                //por cada centroide calculo la distancia a este y veo cual es el que más me conviene
                {
                    ...
                    {
                        float diff =
                            datos_local[i * n_columnas + d] - centroides[j * n_columnas + d];
                        dist += diff * diff;
                    }
                ...
                conteo_thread[mejor_centroide]++;

                for (size_t d = 0; d < n_columnas; ++d)
                {
                    suma_thread[mejor_centroide * n_columnas + d] += datos_local[i * n_columnas + d];
                }
            } //si he terminado mis iteraciones no me detengo y continúo

            //reducción en serie por hilo
            #pragma omp critical
            {
                desplazados_local += desplazados_thread;
                for (size_t i = 0; i < suma_local.size(); ++i)  //actualizo suma
                    suma_local[i] += suma_thread[i];

                for (int i = 0; i < num_centroides; ++i)
                    conteo_local[i] += conteo_thread[i];    //actualizo conteo
            }
        }

        // C. reducción MPI fuera de la zona paralela
        //aquí estou creando dos vectores extra por cada nodo
